...
const addCartToHTML = () => {
    ...
    if(cart.length > 0){
        cart.forEach(item => {
            totalQuantity += item.quantity; // Increment totalQuantity by item quantity
            ...
            `;
            totalPrice += info.price * item.quantity; // Calculate totalPrice
        });
    }
    ...
    // Populate cart details in checkout modal
    ...
    if (cart.length > 0) {
        ...
        cart.forEach(item => {
            let positionProduct = products.findIndex((value) => value.id == item.product_id);
            let info = products[positionProduct];12
            let newItem = document.createElement('div');
            newItem.classList.add('item');
            newItem.dataset.id = item.product_id;
            newItem.innerHTML = `
